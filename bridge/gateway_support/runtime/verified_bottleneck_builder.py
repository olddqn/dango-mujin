"""
verified_bottleneck_builder.py — Verified Bottleneck layer (F-9 / F-11).

Route B entry from a shared Observed Edge: a Verified Bottleneck records that a
gateway's OWN, publicly self-stated operational bottleneck is *currently
observable* — without inference. Per F-11, verified is NOT proof; it is a
"currently observable support condition". The four minimal conditions (all
required): self-stated · public · currently observable · inference-free. If any
is missing the bottleneck is HELD (not recorded).

It consumes an edge_id from the shared Edge Runtime (edge records are not
duplicated here). Person Domain stays sealed: the record concerns the gateway-as-
actor's self-stated public need, never the absent owner's need (no need inference).

CLI:
  python -m bridge.gateway_support.runtime.verified_bottleneck_builder
"""
...
from bridge.edge_runtime.runtime.edge_builder import get_edge
from ..store import (
...
def make_verified_bottleneck(
    ...
    satisfied (the bottleneck would be HELD, not verified) or the edge is unknown.
    T0 (edge_observed_at) is taken from the shared edge unless overridden."""
    ...
    if edge is None:
        raise ValueError(f"unknown edge {edge_id} (verify an Observed Edge first)")
    rec = {
        "record_type": "verified_bottleneck",
        "bottleneck_id": None,  # assigned at persist
        "edge_id": edge_id,                   # shared Observed Edge (not duplicated)
        ...
        "edge_observed_at": edge_observed_at or edge.get("observed_at") or utc_now_iso(),  # T0
...
def record_verified_bottleneck(**kwargs: Any) -> dict[str, Any]:
    """Construct + persist a verified bottleneck (idempotent by edge_id+kind)."""
    rec = make_verified_bottleneck(**kwargs)
    key = (rec["edge_id"], rec["bottleneck_kind"])
    for existing in read_jsonl(VERIFIED_BOTTLENECKS_JSONL):
        if (existing.get("edge_id"), existing.get("bottleneck_kind")) == key:
            return existing
