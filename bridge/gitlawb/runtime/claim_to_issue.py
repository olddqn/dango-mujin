...
def claim_to_issues(claim: dict, condition: str | None = None) -> list[dict]:
    ...
    Scoped prerequisite preference (schema_version 1.1):
    If the claim dict contains a 'scoped_plan_tree_file' key pointing to a
    scoped plan tree JSON, or if the claim itself has 'applicable_prerequisites',
    prefer scoped issue generation via scoped_plan_to_issue.py.
    Falls back to unscoped behavior if scoped data is unavailable.
    """
    # Try scoped generation first if applicable_prerequisites present
    applicable = claim.get("applicable_prerequisites", [])
    bypassed   = claim.get("bypassed_prerequisites", [])
    if applicable or bypassed:
        try:
            from scoped_plan_to_issue import generate_issue_candidate
            results = []
            for cond in applicable:
                ...
                results.append(generate_issue_candidate(claim, cond, scope_info))
            for cond in bypassed:
                if condition and cond != condition:
                    continue
                scope_info = {
                    "condition":    cond,
            ...
            pass  # fall through to unscoped behavior

    # Unscoped fallback: original behavior unchanged
