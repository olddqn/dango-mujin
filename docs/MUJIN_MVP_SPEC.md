# Mujin Platform MVP v0.1 — Specification Freeze

- **Status:** Frozen (v0.1) — 仕様凍結
- **Date:** 2026-06-12
...
| 2 | [`docs/adr/ADR-001-CANONICAL-REALITY-FEEDBACK.md`](adr/ADR-001-CANONICAL-REALITY-FEEDBACK.md) | 参照済み |
| 3 | [`docs/adr/ADR-002-DANGO-MUJIN-BOUNDARY.md`](adr/ADR-002-DANGO-MUJIN-BOUNDARY.md) | 参照済み |
| 4 | [`docs/adr/ADR-003-RELIEF-CASE-PROMOTION.md`](adr/ADR-003-RELIEF-CASE-PROMOTION.md) | 参照済み |
| 5 | [`docs/adr/ADR-004-CONTRIBUTION-MODEL.md`](adr/ADR-004-CONTRIBUTION-MODEL.md) | 参照済み |
| 6 | `claim-saiyan-scouter-001`（Dan-Go Claim） | **リポジトリ／git履歴に未在。** 本指示で与えられた制約（§設計思想・継承制約 1–8）を一次ソースとして扱う。→ Open Question OQ-1 |
| 7 | `mujin_reflections_from_saiyan_scouter` | **同上、未在。** Risks 章（§11）に反映。→ OQ-1 |

> 資料 6・7 はファイルとして存在しないが、その**結論（継承制約と Reach Gap の未解決性）は本仕様の不変条件として完全に取り込む**。原典の復元は OQ-1 として残す。
...
## MVP 全体の不変条件（Saiyan Scouter 由来・凍結）

以下8つは MVP 全章を貫く不変条件であり、いかなる章もこれに反してはならない。
...
5. **成功指標は単一化しない。**
6. **`created_from`（`relief_case` / `direct_application` / `npo_proxy` / `scouter`）による序列を作らない。**
7. **Saiyan Scouter 問題は未解決。** ゆえに MVP に **自動発見・自動スカウト・自動アウトリーチを実装しない。**
...
## 1.1 目的
Mujin MVP v0.1 の目的は、**Dan-Go が公開した決定情報から生まれた共助ケースについて、当事者の同意のもとで協力を可視化し、進捗と現実フィードバックを記録できる最小のプラットフォームを成立させること**である。

## 1.2 成功条件（単一化しない — 不変条件5）
...
- Relief Case → Mujin Case への**手動昇格**フロー（ADR-003: verified → consent_obtained → promoted、human review 必須）。
...
Relief Case (Phase 18, advisory観察)
   │   [ADR-003 昇格ゲート]
   ▼  verified → consent_obtained → promoted   (※ human review 必須・自動化しない)
Promotion
   ▼
...
MujinCase {
  ...
  status:         enum     // open | in_progress | paused | resolved_by_parties | not_pursued
                           //   ※ "not_pursued" は失敗ではない（不変条件2,4）
  // 不変条件（全レコードに付与）
  authority:           "none"
  ...
  contestable:         true
  reopenable:          true
  registration_is_not_proof: true   // 不変条件1
}
```

**凍結ルール:**
- `created_from` の4値に**優先順位・重み・序列を与えない**（不変条件6）。表示順は中立（例: 作成日時）。
- `status` に成功/失敗の含意を持たせない。`not_pursued` / `resolved_by_parties` は中立記録。
...
> 5ページのみ。各ページに「非表示にするもの」を明記する（何を見せないかが仕様）。

| Path | 目的 | 表示内容 | 非表示内容（凍結） |
|---|---|---|---|
| `/` | 入口・思想の提示 | Mujin の目的（寄付サイトではない旨）、進行中ケース数（中立カウント）、透明性への導線 | 成功率・ランキング・「最も支援が必要なケース」等の序列表示 |
| `/cases` | ケース一覧（中立） | 公開許可済みケースの中立リスト（作成日時順）、各ケースの `status`・`needs` 概要 | スコア・優先度・`created_from` バッジによる序列、未同意ケース |
| `/cases/:id` | ケース詳細 | `needs`、`progress_log`（append-only 履歴）、Contribution 状況、`visibility` 範囲内の情報 | 当事者の身元（同意範囲外）、成功/失敗ラベル、vulnerability の定量化 |
| `/contribute` | 協力の記録 | Contribution 型一覧、対象 `need` 選択、`offered/committed` 記録、撤回の説明 | 寄付決済 UI、貢献額ランキング、貢献者スコア |
| `/transparency` | 透明性 | append-only 記録の閲覧、不変条件の明示、Reach Gap が未解決である旨の明記、Mujin を通らない支援も尊重する旨 | 個人特定可能情報、内部判断の権威化表現 |

**全ページ共通の非表示原則:**
- 成功指標の単一化表示をしない（不変条件5）。
- 「なぜこのケースが表示されているか」を説明できない並べ替えをしない（不変条件8 の UI 版）。
- 未同意・撤回済みケースは表示しない（§8）。
...
## 8.4 登録しない自由
- **Mujin に登録しない選択を尊重する**（不変条件1, 4）。登録は支援価値の証明ではない。
- Mujin を通らない支援（直接の相互扶助）を劣位に置く表示をしない。

## 8.5 接触の正当性
- **「なぜあなたに接触したのか」を説明できない接触を正当化しない**（不変条件8）。MVP は能動的アウトリーチを持たない（§9）ため、接触は当事者起点（`direct_application`）または human review 済み昇格（`relief_case`）に限る。
...
> これらは「将来検討」ではあっても、**Saiyan Scouter 問題が未解決である限り MVP には入らない**（不変条件7）。
...
| Phase | 名称 | 内容 | 完了の目安 |
|---|---|---|---|
| **A** | **Data Layer** | Mujin Case スキーマ（§4）の表現、`bridge/mujin/` 配下のレコード形式、Relief Case 参照（非破壊） | Mujin Case が append-only で表現・参照できる |
| **B** | **Export Layer** | Dan-Go Decision 情報（Directive + Execution Log）の読み取り専用エクスポート（ADR-002） | `directive_id` 単位で決定情報を読める |
| **C** | **UI Layer** | 5ページ（§7）の最小実装。非表示原則の順守 | 5ページが中立表示で閲覧可能 |
| **D** | **Feedback Layer** | Reality Feedback の append（ADR-001）、Phase 27 Bridge との接続（自動反映なし） | feedback が正典 JSONL に append され、Bridge が advisory に拾える |

依存順: A → B → C → D（D は A–C に依存）。各 Phase は不変条件8項目を満たすこと。
...
| ID | リスク | 内容 | MVP での扱い |
|---|---|---|---|
| R1 | **自己申請バイアス** | `direct_application` に偏ると、申請できる者だけが可視化される | 成功指標を単一化しない（不変条件5）。Reach Gap 未解決を明記（§12） |
| R2 | **NPO 権力集中** | `npo_proxy` 経由が支配的になり、NPO がゲートキーパー化 | `created_from` に序列を与えない（不変条件6）。proxy も検証・同意ゲートを通す（ADR-003） |
| R3 | **支援者権力** | 貢献者がケースに対し優位・支配を持つ | スコア・ランキング非実装（§9）。Contribution に債務・見返りを持ち込まない（ADR-004） |
| R4 | **成功物語バイアス** | 「成功したケース」だけが語られ、撤回・不成立が不可視化 | Story Agent 非実装（§9）。撤回は失敗ではない（不変条件3）。`not_pursued` を中立記録 |
| R5 | **Reach Gap** | そもそも Mujin に到達できない人が構造的に取り残される | **未解決。MVP は解決を主張しない**（§12 OQ）。自動アウトリーチで"解決した風"にしない（不変条件7） |
...
| ID | Open Question |
|---|---|
| **OQ-1** | `claim-saiyan-scouter-001` と `mujin_reflections_from_saiyan_scouter` の原典がリポジトリに存在しない。原典を正式に取り込む／復元するか。本仕様は与えられた制約要約を一次ソースとして凍結しているが、原典との突合が未了。 |
| **OQ-2** | **Reach Gap をどう扱うか。** 自動アウトリーチを禁じたうえで、到達できない人の存在をどう可視化・尊重するか（解決ではなく誠実な明示の方法）。 |
...
> **Saiyan Scouter 問題は未解決である。Mujin MVP は Reach Gap 問題を解決したと主張しない。**

---

# 13. MVP Completion Checklist

MVP v0.1 が「完成」と判定される条件。**すべて満たして初めて完成**。
...
- [ ] 撤回がいつでも可能で、撤回が評価低下に繋がらない。
- [ ] 「登録しない自由」「Mujin を通らない支援」を尊重する表示。

## 13.4 非実装の保証（§9・不変条件）
- [ ] 自動発見・自動スカウト・自動アウトリーチが**無い**。
- [ ] スコアリング・ランキング・AI 自動判断が**無い**。
- [ ] `created_from` による序列表示が**無い**。
- [ ] 決済・送金が**無い**（`moves_money: false`）。
- [ ] Execution Log / Claim / Directive への書き込みが**無い**。

## 13.5 誠実性（§11, §12）
- [ ] `/transparency` に **Reach Gap が未解決である旨**が明記されている。
- [ ] MVP が Reach Gap を解決したと**主張していない**。
- [ ] Saiyan Scouter 由来の不変条件8項目が全章で破られていない。

---

*本文書は仕様凍結（Specification Freeze）であり、実装・コード・追加提案を含まない。実装は本凍結の承認後に別フェーズで着手する。*
